...
if __name__ == '__main__':
    ...
    pretrain_op = tf.train.AdamOptimizer(new_lr).minimize(contrast_loss)

    # supervised learning
    prob_node = tf.nn.softmax(class_logit)
    x_label = tf.cast(input_label_node, tf.int32)
    class_loss = tf.losses.sparse_softmax_cross_entropy(x_label, class_logit)
    funetrain_op = tf.train.AdamOptimizer(cfg.lr).minimize(class_loss)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        if mode == 'pre_train':
            ...
            while step < cfg.epochs:
                ...
                for k in range(num_batch):
                    ...
                    label_l_cut = train_label_l[s_idx:e_idx]  # label for supervised learning
                    ...
                    _, out_loss, out_learn = sess.run([pretrain_op, contrast_loss, new_lr], feed_dict={
                    ...
                    if (k + 1) % (100) == 0:
                        ...
                        print('step: %d   ——  pretrain_loss: %.4f  —— learning_rate: %.6f' % (
                            (k + 1) // (100), ave_loss, out_learn))
                        ave_loss = 0
                        ave_t1_loss = 0

            if not os.path.exists('./saver/wiki_model'):
                os.makedirs('./saver/wiki_model')

            saver = tf.train.Saver()
            saver.save(sess, './saver/wiki_model/params.ckpt')
        ...
        if mode == 'fune_train':
            ...
            saver = tf.train.Saver()
            sess.run([initialize_op])
            saver.restore(sess, './saver/wiki_model/params.ckpt')
            for i_step in range(cfg.epochs):
                print('total org epoch: %d' % i_step)
                step += 1
                num_instance = len(train_src_l)
                num_batch = math.ceil(num_instance / cfg.batchsize)
                ave_loss = 0
                for k in range(num_batch):
                    ...
                    if (k + 1) % (100) == 0:
                        ...
                        print('step: %d   ——  funetrain_loss: %.4f ' % (
                            (k + 1) // (100), ave_loss))
                        ave_loss = 0
